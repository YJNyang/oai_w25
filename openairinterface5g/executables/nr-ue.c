...
void *UE_thread(void *arg) {
  ...
  while (!oai_exit) {
    ...
    if (UE->UE_fo_compensation){
      ...
      double off_angle = -2*M_PI*s_time*(UE->trace_syn_fre_offset);  // ue->trace_syn_fre_offset为跟踪同步时计算得到的频偏（每帧一次）

      // start for offset correction
      int start = 0;
      // loop over samples
      int end = readBlockSize;

      for(int n=start; n<end; n++){
        for (int ar=0; ar<UE->frame_parms.nb_antennas_rx; ar++) {
          re = ((double)(((short *)rxp[ar]))[2*n]);
          im = ((double)(((short *)rxp[ar]))[2*n+1]);
          ((short *)rxp[ar])[2*n] = (short)(round(re*cos(n*off_angle) - im*sin(n*off_angle)));
          ((short *)rxp[ar])[2*n+1] = (short)(round(re*sin(n*off_angle) + im*cos(n*off_angle)));
        }
      }
    }
    //++++++++++++++++++++++++++++++++++end
