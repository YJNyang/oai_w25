...
void nr_decode_pucch0(PHY_VARS_gNB *gNB,
  ...
  int SNRtimes10;//add_yjn_harq
  static int sigenergy=0;//add_yjn_harq
   if (pucch_pdu->bit_len_harq==0) uci_pdu->rssi = 1280 - (10*dB_fixed(32767*32767))-dB_fixed_times10(sigenergy);
   else{//add_yjn_harq
  for (int aa=0;aa<frame_parms->nb_antennas_rx;aa++)
    // sigenergy += signal_energy_nodc(rp[aa][0],12);
    sigenergy = signal_energy_nodc(rp[aa][0],12);//add_yjn_harq
  uci_pdu->rssi = 1280 - (10*dB_fixed(32767*32767))-dB_fixed_times10(sigenergy);
   }
  SNRtimes10 = xrtmag_dBtimes10-(10*max_n0);
  // int cqi;
  static int cqi = 0;//add_yjn_harq
  if (pucch_pdu->bit_len_harq==0)  cqi =cqi;//add_yjn_harq
  else if (SNRtimes10 < -640) cqi=0;
  ...
  if (pucch_pdu->bit_len_harq==0) no_conf=false; //add_yjn_harq
  ...
  if (pucch_pdu->bit_len_harq==0) {
    ...
    uci_pdu->harq->harq_list[0].harq_value = 0;//add_yjn_harq
    ...
    uci_pdu->harq->harq_list[1].harq_value = 0;//add_yjn_harq
    uci_pdu->harq->harq_list[0].harq_value = 0;//add_yjn_harq
